...
abstract class CookiesTest(private val factory: HttpClientEngineFactory<*>) : TestWithKtor() {
    ...
    fun multipleClients() = runBlocking {
        ...
        val a = client.config { install(HttpCookies) { default { runBlocking { addCookie("localhost", Cookie("id", "1")) } } } }
        val b = a.config { install(HttpCookies) { default { runBlocking { addCookie("localhost", Cookie("id", "10")) } } } }
        val c = a.config { }
        val d = b.config { }
